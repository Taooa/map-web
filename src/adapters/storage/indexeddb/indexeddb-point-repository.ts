...
async function collectSourceOptions(store: IDBObjectStore): Promise<readonly string[]> {
  const sources = new Set<string>();
  const cursorRequest = store.openCursor();
  await new Promise<void>((resolve, reject) => {
    cursorRequest.addEventListener('error', () =>
      reject(cursorRequest.error ?? new Error('读取点位来源失败。')),
    );
    cursorRequest.addEventListener('success', () => {
      ...
      sources.add(sourceName(cursor.value as Point));
      cursor.continue();
    });
  });
...
export class IndexedDBPointRepository implements PointRepository {
  ...
  async listPage(query: PointListQuery): Promise<RepositoryResult<PointListPage>> {
    try {
      ...
      if (!hasFilters) {
        const sourceOptionsPromise = collectSourceOptions(store);
        ...
        const pointIds = includePointIds
          ...
          : [];
        const total = includePointIds ? pointIds.length : await requestToPromise(store.count());
        const points: Point[] = [];
        const cursorRequest = store.index(field).openCursor(undefined, direction);
        await new Promise<void>((resolve, reject) => {
          let advanced = offset === 0;
          cursorRequest.addEventListener('error', () =>
            reject(cursorRequest.error ?? new Error('读取点位分页失败。')),
          );
          cursorRequest.addEventListener('success', () => {
            const cursor = cursorRequest.result;
            if (!cursor || points.length >= limit) {
              resolve();
              return;
            }
            if (!advanced) {
        ...
        return success({
          ...
          pointIds,
          total,
          sourceOptions: await sourceOptionsPromise,
        });
      }
