...
Coordinate Toolkit 是一个浏览器本地运行的设备点位管理、坐标转换与地图核验工作台。当前应用已具备点位录入/导入、本地持久化、WGS84/GCJ02/BD09 转换缓存，以及高德、百度、天地图统一工作台。地图页以全尺寸地图为主体，提供悬浮点位工作区、平台工具和基础地图控制。
...
地图工作区通过“添加点位”弹窗从仓库选择点位，支持名称/来源筛选、分页、跨页选择和全选全部查询结果。已加入工作区的点位不会重复出现在候选列表；左侧勾选仅控制 Marker 显隐，不会删除或移出原始点位数据。
